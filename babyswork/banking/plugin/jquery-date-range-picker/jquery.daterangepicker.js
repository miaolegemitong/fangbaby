				...
				var offset = $(self).offset();
						if ($(opt.container).css("position") == "relative")
						{
							...
							box.css(
							{
								...
								left: Math.max(offset.left - containerOffset.left,10)
							});
						}
						else
						{
							...
							else
							{
								...
								{
									top: offset.top+$(self).outerHeight() + parseInt($('body').css('border-top') || 0,10 ),
									left: Math.max(offset.left + $(self).width() - box.width() - 16,10)
								});
							}
						}
			}
		...
		function createMonthHTML(d)
		{
			...
			for(var week=0; week<6; week++)
			{
				...
				for(var day = 0; day<7; day++)
				{
					...
					html.push('<td class="'+getBanjia(today.time)+'" ' + attributesCallbacks({},opt.dayTdAttrs,today) + '><div ' + attributesCallbacks(todayDivAttr,opt.dayDivAttrs,today) + '>'
						+showDayHTML(today.time, today.day)
						+' <span style="display:block;margin-top:3px;width:48px">'+ getLunarDispaly(today.time)+'</span> '
						+' </div></td>');
					//console.log(today.time, today.day)
				}
				html.push('</tr>');
			}
...
var han_number = ['','初一','初二','初三','初四','初五','初六','初七','初八','初九','初十','十一','十二','十三','十四','十五','十六','十七','十八','十九','二十',
...
function getLunarDispaly(ti){
	var lu_test = new Lunar(ti);
	var lday = parseInt(lu_test.day);
	var lmonth = parseInt(lu_test.month);
	var lyear = parseInt(lu_test.year);

	var lkey = (lmonth<10 ? "0"+lmonth : lmonth)+"" + (lday<10 ? "0"+lday : lday);
	var llongkey = (lmonth<10 ? "0"+lmonth : lmonth)+"-" + (lday<10 ? "0"+lday : lday);
	...
	var key = (month<10 ? "0"+month : month)+"" + (day<10 ? "0"+day : day);
	var longkey = (month<10 ? "0"+month : month)+"-" + (day<10 ? "0"+day : day);

	var label = "";
	label = lljr[lkey];
	label = label ? label : yljr[key];

	if(!label){
		var lunar = window.banjie['lunar'];
		if(lunar && lunar[llongkey]){
			label = lunar[llongkey];
			return label;
		}
		var solar = window.banjie['solar'];
		if(solar && solar[longkey]){
			label = lunar[longkey];
			return label;
		}
		if(window.banjie['vication']){
			var vication = window.banjie['vication'];
			var fullkey = year+"-"+longkey;
	...
	if(!label){
		label = han_number[lday];
	}
	return label;
}

function getBanjia(ti){
	var _date = new Date();
	_date.setTime(ti);
	var day = _date.getDate();
	var month = _date.getMonth()+1;
	...
	if(window.banjie['holiday'] && window.banjie['holiday'][key]){
		return "jia";
	}
	if(window.banjie['work'] && window.banjie['work'][key]){
		return "ban";
	}
	return "wu";
}
...
$.ajax({
	type : 'GET',
	url: 'http://awp-assets.meituan.net/hfe/dashboard/holiday/config.json',
	cache:false,
	datType : 'json',
